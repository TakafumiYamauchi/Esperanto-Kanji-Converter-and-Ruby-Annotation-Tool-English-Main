# JSON File Generation Page for Esperanto Text (Kanji) Replacement.py (2つ目)
...
#=====================================================================
st.set_page_config(
    page_title="Tool for Generating the JSON File for Esperanto Text (Kanji) Replacement",
    layout="wide"
)
st.title("Generate a JSON File for Kanji Replacement in Esperanto Text")
st.write("---")

#=====================================================================
# 2) 概要説明 (使い方)
#=====================================================================
with st.expander("How to Use (Open Guide)", expanded=True):
    st.markdown("""
    #### Introduction
    On this page, you can generate the **replacement JSON file** (which might be around 50MB)
    used for **Esperanto text (Kanji) replacement** in the main page, and then download it.

    **Procedure:**
    1. Upload or use the default **CSV file** (it contains correspondence of Esperanto roots and translations).
    2. Upload or use the default **JSON file** (it includes custom rules for word-stemming and replacement strings).
    3. Click the button “Create the replacement JSON file” to generate 
       the **replacement JSON file**, and download it.

    Sample files are also provided below for reference on how to customize your own CSV or JSON format.
    """)

#=====================================================================
# 3) サンプルファイル一覧 (折りたたみ)
#=====================================================================
with st.expander("Sample Files for Download"):
    st.write("#### List of Sample Files")

    # サンプルCSV１
    st.markdown("""
    **Sample CSV 1 (Esperanto root – English translation/ruby)**  
    This CSV file contains line-by-line correspondence between an Esperanto root and an English translation.  
    """)
    file_path0 = './Appの运行に使用する各类文件/Esperanto Roots - English Translation Ruby Correspondence List.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Download Sample CSV 1 (Esperanto root – English translation/ruby)",
            data=file,
            file_name="Esperanto Roots - English Translation Ruby Correspondence List.csv",
            mime="text/csv"
        )

    # サンプルCSV２
    st.markdown("""
    **Sample CSV 2 (Esperanto root – Kanji correspondence; Mingeo’s proposal)**  
    This CSV file pairs Esperanto roots with Kanji.
    """)
    file_path0 = './Appの运行に使用する各类文件/Mingeo先生版 世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Download Sample CSV 2 (Esperanto root – Kanji correspondence by Mingeo)",
            data=file,
            file_name="Esperanto root – Kanji correspondence by Mingeo.csv",
            mime="text/csv"
        )

    # サンプルCSV３
    st.markdown("""
    **Sample CSV 3 (Esperanto root – Kanji correspondence)**  
    Another CSV file linking Esperanto roots to Kanji.
    """)
    file_path0 = './Appの运行に使用する各类文件/世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Download Sample CSV 3 (Esperanto root – Kanji correspondence)",
            data=file,
            file_name="Esperanto root – Kanji correspondence.csv",
            mime="text/csv"
        )

    # サンプルJSON１
    st.markdown("""
    **Sample JSON 1 (User-defined rules for Esperanto word-stemming)**  
    **Usage**: This file shows how to segment Esperanto words into their roots,
    or how to handle verb endings, etc.  
    For example, `"["am", "dflt", ["verbo_s1"]]"` can define certain rules. 
    """)
    ...
    with open(json_file_path, "rb") as file_json:
        btn_json = st.download_button(
            label="Download Sample JSON 1 (User-defined rules for Esperanto word-stemming)",
            data=file_json,
            file_name="User-defined rules for Esperanto word-stemming.json",
            mime="application/json"
        )

    # サンプルJSON２
    st.markdown("""
    **Sample JSON 2 (User-defined replacement strings)**  
    **Usage**: Assign your own Kanji or special notation for specific words, 
    in addition to the above word-stemming rules. 
    (Often, just editing the CSV and using the word-stemming JSON is enough for typical use.)
    """)
    json_file_path2 = './Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json'
    with open(json_file_path2, "rb") as file_json:
        btn_json = st.download_button(
            label="Download Sample JSON 2 (User-defined replacement strings)",
            data=file_json,
            file_name="User-defined replacement strings.json",
            mime="application/json"
        )

    # サンプルExcel
    st.markdown("""
    **Sample Excel 1 (Esperanto root – Japanese translation/ruby list with proficiency level)**  
    **Usage**: Useful for customizing which Esperanto roots to add translation/ruby to. 
    Includes a “proficiency level” from an Esperanto-Japanese dictionary.
    """)
    with open('./Appの运行に使用する各类文件/エスペラント語根-日本語訳ルビ対応リスト(習得レベル付き).xlsx', "rb") as file:
        st.download_button(
            label="Download Sample Excel 1 (Esperanto root – Japanese translation/ruby list, with level)",
            data=file,
            file_name="Esperanto root – Japanese translation/ruby list, with level.xlsx",
...
format_type_options = {
    ...
    "Simple replacement only": "替换后文字列のみ(仅)保留(简单替换)"
}
display_format_type_options = list(format_type_options.keys())
selected_display = st.selectbox(
    "Choose the output format:",
    display_format_type_options
)
format_type = format_type_options[selected_display]
...
st.write("---")
st.markdown("**Sample of formatted text ↓**")
...
#=====================================================================
st.header("Step 1: Prepare Your CSV File")
st.markdown(
    """
### Select a **CSV file** containing Esperanto roots and translations (Kanji, etc.)
    
---
    """
)

csv_choice_display_map = {
    "Upload a CSV file": "アップロードする",
    "Use the default CSV file": "デフォルトを使用する"
}
csv_choice_display = st.radio(
    "How do you want to handle the CSV file?",
    list(csv_choice_display_map.keys())
)
csv_choice = csv_choice_display_map[csv_choice_display]
csv_path_default = "./Appの运行に使用する各类文件/Esperanto Roots - English Translation Ruby Correspondence List.csv"

CSV_data_imported = None

if csv_choice == "アップロードする":
    st.write("Please upload your CSV file (UTF-8 recommended).")
    ...
    if uploaded_file is not None:
        ...
        st.success("CSV file has been uploaded successfully.")
    else:
        st.warning("No CSV file was uploaded.")
        st.stop()
    ...
    try:
        ...
        csv_buffer = StringIO(converted_text)
        CSV_data_imported = pd.read_csv(csv_buffer, encoding="utf-8", usecols=[0, 1])
        st.info("Using the default CSV file.")
    except FileNotFoundError:
        st.error("The default CSV file was not found. Process aborted.")
        st.stop()

st.write("The CSV file has been successfully read. Proceed to the next step.")
st.write("---")

#=====================================================================
# ステップ2: JSONファイル(語根分解法など)を準備
#=====================================================================
st.header("Step 2: Prepare the JSON Files (Word-Stemming Rules, etc.)")
st.markdown("""
You can upload or use the default JSON files that define 
**custom Esperanto word-stemming rules** or **custom replacement strings**.
Refer to the sample JSON files for the general format.
""")

json_choice_display_map = {
    "Upload a JSON file": "アップロードする",
    "Use the default JSON file": "デフォルトを使用する"
}
json_choice_display = st.radio(
    "1. How would you like to handle the JSON file for additional Esperanto word-stemming rules?",
    list(json_choice_display_map.keys())
)
json_choice = json_choice_display_map[json_choice_display]

json_path_default = "./Appの运行に使用する各类文件/世界语单词词根分解方法の使用者自定义设置.json"
custom_stemming_setting_list = None

if json_choice == "アップロードする":
    uploaded_json = st.file_uploader("Please upload the JSON file", type=['json'])
    if uploaded_json is not None:
        custom_stemming_setting_list = json.load(uploaded_json)
        st.success("The JSON file has been uploaded successfully.")
    else:
        st.warning("No JSON file was uploaded.")
        st.stop()
elif json_choice == "デフォルトを使用する":
    try:
        with open(json_path_default, "r", encoding="utf-8") as g:
            custom_stemming_setting_list = json.load(g)
        st.info("Using the default JSON file for word-stemming rules.")
    except FileNotFoundError:
        st.error("The default JSON file was not found.")
        st.stop()

json_choice2_display_map = {
    "Upload a JSON file": "アップロードする",
    "Use the default JSON file": "デフォルトを使用する"
}
json_choice2_display = st.radio(
    "2. How would you like to handle the JSON file for additional replacement strings?",
    list(json_choice2_display_map.keys())
)
json_choice2 = json_choice2_display_map[json_choice2_display]
...
if json_choice2 == "アップロードする":
    uploaded_json = st.file_uploader("Please upload the second JSON file", type=['json'])
    if uploaded_json is not None:
    ...
    try:
        ...
        st.info("Using the default JSON file for replacement strings.")
    except FileNotFoundError:
        st.error("The default JSON file was not found.")
        st.stop()
...
# ステップ3: 高度な設定 (並列処理)
#=====================================================================
st.header("Step 3: Advanced Settings (Parallel Processing)")
with st.expander("Open the parallel processing settings"):
    st.write("""
    Here, you can configure the number of processes (parallel processing) 
    for generating the replacement JSON file. This can speed up the process 
    if there is a large amount of text or many roots.
    """)

    use_parallel = st.checkbox("Use parallel processing", value=False)
    num_processes = st.number_input("Number of parallel processes", min_value=2, max_value=6, value=5, step=1)

st.write("### Generate the final replacement JSON file (button below)")

#=====================================================================
# 置換用JSONファイルを作成するボタン
#=====================================================================
if st.button("Create the replacement JSON file"):
    with st.spinner("Generating the replacement JSON file... Please wait a moment."):
        #-------------------------------------------------------------
        ...
        #-------------------------------------------------------------
        # (3) CSV_data_imported を用いて、翻訳文字列を反映させる
        #-------------------------------------------------------------
        ...
        #-------------------------------------------------------------
        # (5) placeholder(占位符)を割り当て (old→placeholder→new)
        #-------------------------------------------------------------
        ...
        #-------------------------------------------------------------
        # (6) E_stem_with_Part_Of_Speech_list へ適用
        #-------------------------------------------------------------
        if use_parallel:
            ...
            for i, j in enumerate(E_stem_with_Part_Of_Speech_list):
                ...
                if i % 1000 == 0:
                    ...
                    progress_text.write(f"{current_count}/{total_items} items processed...")

            progress_bar.progress(100)
            progress_text.write("Time-consuming step is 100% complete (please wait a few more seconds).")
        ...
        st.success("Creation of the replacement list is complete!")

        st.download_button(
            label="Download the final replacement list (merged 3 JSON files)",
            data=download_data,
            file_name="final replacement list (merged 3 JSON files).json",
            mime='application/json'
        )
